...
class MockUserRepository(UsersRepositoryInterface):
    def getUserByUsername(self, username: str) -> Optional[User]:
        if username == "existing_user":
            return User(username="existing_user", firstName="First", lastName="Last", passwordHash="hashed_password", role=UserRole.ADMIN)
        return None
...
def test_authenticate_user_existing_user(mock_user_repository, mock_password_hasher):
    auth_service = AuthenticationService(mock_user_repository, mock_password_hasher)
    result = auth_service.authenticateUser("existing_user", "password")
    assert result is True

def test_authenticate_user_non_existing_user(mock_user_repository, mock_password_hasher):
    auth_service = AuthenticationService(mock_user_repository, mock_password_hasher)
...
def test_generate_username(mock_user_repository, mock_password_hasher):
    ...
    with patch("app.services.authentication_service.random.randint", return_value=123):
