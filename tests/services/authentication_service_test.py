...
def test_authenticate_user_existing_user(mock_user_repository, mock_password_hasher):
    ...
    result = auth_service.authenticateUserCredentials("existing_user", "password")
    assert result is True


def test_authenticate_user_non_existing_user(
    mock_user_repository, mock_password_hasher
):
    auth_service = AuthenticationService(mock_user_repository, mock_password_hasher)
    result = auth_service.authenticateUserCredentials("non_existing_user", "password")
